...
void Mul_Matrix(float *A,float *B,float *C,int m1,int n1,int n2){
	...
	for(int i=0;i<m1;i++){
		v4 = _mm256_setzero_ps();
		for(int j=0;j<n1;j++){
			v1 = _mm256_set1_ps(A[i*n1+j]);
			for(int k=0;k<k_max;k+=8){
				v2 = _mm256_loadu_ps(&B[j*n2+k]);
               				v3 = _mm256_loadu_ps(&C[i*n2+k]);
				v4 = _mm256_fmadd_ps(v1,v2,v3);
				_mm256_storeu_ps(&C[i*n2+k],v4);
			}
