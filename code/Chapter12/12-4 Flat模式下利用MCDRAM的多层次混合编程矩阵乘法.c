...
int main(int argc, char *argv[]){
    ...
    x = coord[0];
    y = coord[1];
    a = (float*)hbw_malloc(sizeof(float)*num*num);  //为各个进程分配对应部分块矩阵的空间
    b = (float*)hbw_malloc(sizeof(float)*num*num);
    c = (float*)hbw_malloc(sizeof(float)*num*num);
    if (id==0){
        A = (float *)hbw_malloc(sizeof(float)*DIMS*DIMS);
        B = (float *)hbw_malloc(sizeof(float)*DIMS*DIMS);
        C = (float *)hbw_malloc(sizeof(float)*DIMS*DIMS);
    ...
    MPI_Comm_hbw_free(&MPI_COMM_CART);
	if (id==0){
	  /*
	  printf("A:\n");
...
void Verify_matrix_c(float *A,float * B,float *C,int m1,int n1,int n2){
    float *matrixPool;
    int i;
    matrixPool=(float *)hbw_malloc(sizeof(float)*m1*n2);
